    ...
    requestZValues(originalFeature) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!this._initialized)
                this._init();
            const { sampledCoords: sampleCoords, gridPolygons } = this._sampleFeatureCoords(originalFeature);
            let contourCoords, mainCoords;
            const source = this.get('source');
            if (typeof source === 'function') {
                // Use a custom function. Useful for using apis to retrieve the zvalues
                ({ mainCoords, contourCoords } = yield source(originalFeature, sampleCoords));
            }
            else {
