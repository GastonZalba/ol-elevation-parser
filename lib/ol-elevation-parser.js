...
class ElevationParser extends Control {
    constructor(options$1) {
        ...
        if (!(_options.source instanceof TileWMS) &&
        ...
        this.set('sampleSizeArea', _options.sampleSizeArea, 
        /* silent = */ true);
    ...
    _addPropertyEvents() {
        ...
        this.on('change:source', (evt) => {
            ...
            if (!(source instanceof Function) &&
                this.get('calculateZMethod') !== 'getFeatureInfo') {
                this._readFromImage = new ReadFromImage(this.get('source'), this.get('calculateZMethod'), this._map);
            }
            else {
    ...
    _sampleFeatureCoords(drawFeature, contour = false) {
        ...
        if (geom instanceof Point) {
            ...
            grid = getPolygonSamples(polygonFeature, this._map.getView().getProjection().getCode(), this.get('sampleSizeArea'));
            if (contour) {
