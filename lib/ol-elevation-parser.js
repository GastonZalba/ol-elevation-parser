    ...
    requestZValues(originalFeature, contour = false) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this._initialized)
                this._init();
            const { coords, gridPolygons } = this._sampleFeatureCoords(originalFeature, contour);
            let coordsWithZ = [];
            const zValues = [];
 ...
 */
    _sampleFeatureCoords(feature, contour = false) {
        const geom = feature.getGeometry();
        let gridPolygons, coords; // For polygons
        if (geom instanceof Point) {
            ...
            if (contour) {
                ...
                coords = gridPolygons.map((g) => g.getGeometry().getInteriorPoint().getCoordinates());
            }
        }
