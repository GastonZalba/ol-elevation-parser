...
class ElevationParser extends Control {
    constructor(options$1) {
         ...
         * @private
         */
        this._getZFromSampledCoords = (coords) => __awaiter(this, void 0, void 0, function* () {
     ...
     * @public
     */
    getElevationValues(feature) {
        return __awaiter(this, void 0, void 0, function* () {
            // Run once
            if (!this._initialized)
                this._init();
            const { sampledCoords: sampleCoords, gridPolygons } = this._sampleFeatureCoords(feature);
            let contourCoords, mainCoords;
            const source = this.get('source');
            if (typeof source === 'function') {
                // Use a custom function. Useful for using apis to retrieve the zvalues
                ({ mainCoords, contourCoords } = yield source(feature, sampleCoords));
            }
            else {
                mainCoords = yield this._getZFromSampledCoords(sampleCoords.mainCoords);
                // Only Polygons
                if (sampleCoords.contourCoords) {
                    contourCoords = yield this._getZFromSampledCoords(sampleCoords.contourCoords);
                }
            }
